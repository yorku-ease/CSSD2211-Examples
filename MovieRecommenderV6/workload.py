...
RATING_URL = "http://127.0.0.1:31000"
RECOMMEND_URL = "http://127.0.0.1:31001"
...
def user_worker(username, actions):
    ...
    for movie, rating in actions:
        ...
        requests.post(f"{RATING_URL}/api/rate", json=payload)
        count += 1

        if count % 3 == 0:
            requests.get(f"{RECOMMEND_URL}/api/recommend/{username}")
